...
Этот репозиторий — консольный TUI-пайплайн для аналитики закупок. Пользователь задаёт вопрос → модель строит **План** (макс. 3 шага) → далее будут выполняться шаги (SQL, API, синтез). Сейчас реализован этап **проверок окружения** и **Планирование**; следующие шаги добавим итеративно.
...
- Язык/стек: Python 3.10+, Textual (TUI), requests, psycopg2-binary, rich.
- Точка входа: `procure-pipeline-tui/main_tui.py`.
- Что уже есть:
  ...
  - Панель **LLM-мета** (модель/опции/метрики, если возвращаются).
  - Кнопки: «Далее» (пауза), «Прервать», «Очистить».
...
```bash
pip install textual rich requests psycopg2-binary python-dotenv
```
...
```bash
python procure-pipeline-tui/main_tui.py
```
...
Ошибка → пользовательское сообщение и запись в лог, TUI не падает.

---

## Архитектура и файлы

- `procure-pipeline-tui/main_tui.py` — консольный TUI (Textual): проверки окружения, ввод вопроса, план от LLM, панель метрик, управление шагами.
- `./logs/` — JSONL-журналы.
